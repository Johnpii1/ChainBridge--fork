...
function generateTypeScriptInterface(schema: SchemaInfo): string {
  ...
  for (const [propName, propInfo] of Object.entries(schema.properties)) {
    ...
    const comment = propInfo.constraints?.length > 0 
      ? ` // ${propInfo.constraints.join(', ')}`
      : '';
...
function generateTypeScriptFile(schemas: SchemaInfo[]): string {
  ...
  lines.push('  listHTLCS(params?: { page?: number; limit?: number }): Promise<ApiResponse<PaginatedResponse<HTLCResponse>>>;');
  lines.push('');
  ...
  lines.push('  listOrders(params?: { page?: number; limit?: number }): Promise<ApiResponse<PaginatedResponse<OrderResponse>>>;');
  lines.push('');
  lines.push('  // Fee operations');
  lines.push('  estimateFees(data: FeeEstimateRequest): Promise<ApiResponse<SwapFeeBreakdownResponse>>;');
  lines.push('  getExchangeRates(data: ExchangeRateRequest): Promise<ApiResponse<RateQuoteResponse>>;');
  lines.push('');
...
async function generateTypes() {
  try {
    ...
    const backendDir = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'backend', 'app', 'schemas');
    const schemaFiles = [
      'htlc.py',
      'order.py', 
      'swap.py',
