...
export class StellarFeeAdapter extends BaseFeeAdapter {
  ...
  constructor(network: 'mainnet' | 'testnet') {
    ...
    this.stellarRpcUrl = network === 'mainnet'
...
export class BitcoinFeeAdapter extends BaseFeeAdapter {
  ...
  constructor(network: 'mainnet' | 'testnet') {
    ...
    super(network, chainInfo);
    this.mempoolApiUrl = network === 'mainnet'
...
export class EthereumFeeAdapter extends BaseFeeAdapter {
  ...
  constructor(network: 'mainnet' | 'testnet', apiKey?: string) {
    ...
    super(network, chainInfo);
    this.etherscanApiUrl = network === 'mainnet'
      ? 'https://api.etherscan.io/api'
      : 'https://api-sepolia.etherscan.io/api';
    this.apiKey = apiKey || (typeof window !== 'undefined' && (window as any).__CHAINBRIDGE_CONFIG__?.etherscanApiKey) || '';
  }
...
export class FeeUtils {
  ...
  static estimateTransactionCost(
    ...
    return {
      ...
      currency: feeEstimate.feeUnit === 'gwei' ? 'ETH' : 
               feeEstimate.feeUnit === 'sat/vB' ? 'BTC' : 'XLM',
    };
  }

  static getFeeRecommendation(feeEstimate: FeeEstimate, urgency: 'low' | 'medium' | 'high'): {
    fee: number;
